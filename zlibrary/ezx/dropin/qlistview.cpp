/****************************************************************************
** $Id: qt/src/widgets/qlistview.cpp   2.3.8   edited 2004-08-05 $
**
...
extern bool qpe_singleFocusWidget(const QWidget *in);
#endif
...
#ifdef QT_KEYPAD_MODE
    if ( !listView()->isModalEditing() && !qpe_singleFocusWidget(listView()) )
				    ...
				    int cx, int cy, int cw, int ch )
{
	...
	switch(p->device()->devType()){
		case QInternal::Widget:
			std::cout<<"p->device()->devType():QInternal::Widget\n";
			break;
		case QInternal::Pixmap:
			std::cout<<"p->device()->devType():QInternal::Pixmap\n";
			break;
		case QInternal::Printer:
			std::cout<<"p->device()->devType():QInternal::Printer\n";
			break;
		case QInternal::Picture:
			std::cout<<"p->device()->devType():QInternal::Picture\n";
			break;
		default:
...
#ifdef QT_KEYPAD_MODE
    if ( isMultiSelection() && i->isSelectable() && ( e->ascii() == ' ' || e->key() == Key_Select) ) {
#else
...
#ifdef QT_KEYPAD_MODE
	  else if ( i && (i->childCount() || i->isExpandable()) ) {
	    i->setOpen( !i->isOpen() );
	    return;
	} else if ( i ) {
	    activatedByClick = FALSE;
