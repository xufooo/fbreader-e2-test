...
std::string ZLEncodingConverter::encodingDescriptionPath() {
	std::cout<<"encodingDescriptionPath():"<<ZLApplication::ZLibraryDirectory() + ZLApplication::PathDelimiter + "encodings\n";
	return ZLApplication::ZLibraryDirectory() + ZLApplication::PathDelimiter + "encodings";
//	return "." + ZLApplication::PathDelimiter + "encodings";
}
...
#if 0
    ZLUnicodeUtil::Ucs2String ustr;
    printf("convert: [%s] %d\n", srcStart, srcEnd - srcStart);
    int len = ZLUnicodeUtil::utf8Length(srcStart, srcEnd - srcStart);
    ZLUnicodeUtil::utf8ToUcs2(ustr, srcStart, srcEnd - srcStart, len);
    for (int i = 0; i < len ;i++) {
	char tmp[4];
	int tmplen;
	if (ustr[i] != 0x2014) {
	    tmplen = ZLUnicodeUtil::ucs2ToUtf8(tmp, ustr[i]);
	} else {
	    tmp[0] = ' ';
	    tmplen = 1;
	}
	dst.append(tmp, tmplen);
    }
#else
