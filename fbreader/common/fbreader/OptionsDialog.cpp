...
const std::string &DefaultLanguageEntry::name() const {
  static const std::string _name = "Language For\nEncoding Detection";
  return _name;
}
...
OptionsDialog::OptionsDialog(FBReader &fbreader, ZLPaintContext &context) {
  ...
  generalTab.addOption(new ZLSimpleBooleanOptionEntry("Quit On Cancel", fbreader.QuitOnCancelOption));
  generalTab.addOption(new ZLSimpleStringOptionEntry("Book Path", fbreader.myCollectionView->collection().PathOption));
  generalTab.addOption(new ZLSimpleBooleanOptionEntry("Look In Subdirs", fbreader.myCollectionView->collection().ScanSubdirsOption));
  generalTab.addOption(new ZLSimpleSpinOptionEntry("Recent Books\nList Size", fbreader.myRecentBooksView->lastBooks().MaxListSizeOption, 1));
  generalTab.addOption(new ZLSimpleBooleanOptionEntry("Keep Pos In Contents", fbreader.StoreContentsPositionOption));
  generalTab.addOption(new ZLSimpleSpinOptionEntry("Delay Between\nKey Pressings", fbreader.KeyDelayOption, 50));

  ZLDialogContent &generalTab2 = myDialog->createTab("Language");
  generalTab2.addOption(new DefaultLanguageEntry());
  ...
  marginTab.addOption(new ZLSimpleSpinOptionEntry("Left Margin", TextStyleCollection::instance().baseStyle().LeftMarginOption, 1));
  marginTab.addOption(new ZLSimpleSpinOptionEntry("Right Margin", TextStyleCollection::instance().baseStyle().RightMarginOption, 1));
  marginTab.addOption(new ZLSimpleSpinOptionEntry("Top Margin", TextStyleCollection::instance().baseStyle().TopMarginOption, 1));
  marginTab.addOption(new ZLSimpleSpinOptionEntry("Bottom Margin", TextStyleCollection::instance().baseStyle().BottomMarginOption, 1));
